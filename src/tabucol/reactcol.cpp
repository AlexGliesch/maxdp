...
namespace gCol {
  namespace pctc {
    ...
    long reactcol(const Graph& g, int *c, int k, unsigned long long maxChecks, int staticTenure, int verbose, int freq, int inc, int **neighbors) {
	...
	if (totalIterations == nextVerbose && incVerbose) {
      ...
      vstream(2) << "          -> Iteration " << totalIterations << " Cost = " << bestSolutionValue << endl;
