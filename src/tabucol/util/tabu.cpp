...
namespace gCol {  
  ...
  int tabu(const Graph & g, vector<int>& c, int k, int maxIterations, int verbose, int **neighbors) {
    ...
    int tabuTenure = 5;		//This is effectively a random choice
    ...
    totalConflicts /= 2;
    vstream(2) << "Initialized the arrays. #Conflicts = " << totalConflicts << endl;
	...
	for (int color = 1; color <= k; color++) {
      ...
      if (bestNode == -1) {
	bestNode = rand() % g.n;
	while ((bestColor = (rand() % k) + 1) != c[bestNode]); // TBD: like this in original code; check logic!
	//
	{
	  numConfChecks += 2;
      ...
      if (verbose > 2) {
	cout << "Will move node " << bestNode << " to color " << bestColor << " with value " << bestValue << " oldconf = " <<
	  conflicts[c[bestNode]][bestNode] << " newconf = " << conflicts[bestColor][bestNode] << " totalConflicts = " << totalConflicts << endl;
      }

      int tTenure = tabuTenure;
      moveNodeToColorForTabu(bestNode, bestColor, g, c, nodesByColor, conflicts, nbcPosition, neighbors, nodesInConflict, confPosition, tabuStatus,
			     totalIterations, tTenure);
	...
	nextVerbose += incVerbose;
      ...
      vstream(2) << "          -> Iteration " << totalIterations << " Cost = " << totalConflicts << endl;
