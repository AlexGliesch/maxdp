...
namespace gCol {
  namespace pctc {
    ...
    inline void greedyCol(vector<vector<int>>& candSol, vector<int>& colNode, const Graph& g, vector<vector<int>>& adjList) {
    ...
    inline void DSaturCol(vector<vector<int>>& candSol, vector<int>& colNode, const Graph& g, vector<vector<int>>& adjList) {
      int i, j, r;
      bool foundColour;
	...
	foundColour = false;
	assignAColourDSatur(foundColour, candSol, g, permutation, nodePos, satDeg, adjList, colNode);
	if (!foundColour) {
