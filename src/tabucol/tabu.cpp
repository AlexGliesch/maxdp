...
long tabu(const Graph& g, int* c, int k, unsigned long long maxChecks,
          unsigned long long maxIterations, int staticTenure, int verbose,
          int freq, int inc, int** neighbors) {
  ...
  while (numConfChecks < maxChecks) {
    ...
    for (int iNode = 1; iNode <= nodesInConflict[0]; iNode++) {
      ...
      for (int color = 1; color <= k; color++) {
        if (color != c[node]) {
          numConfChecks += 2;
          int newValue = totalConflicts + conflicts[color][node] -
                         conflicts[c[node]][node];
          if (newValue <= bestValue && color != c[node]) {
            if (newValue < bestValue) numBest = 0;
            // Only consider the move if it is not tabu or leads to a new very
            // best solution seen globally.
    ...
    if (bestNode == -1) {
      ...
      while ((bestColor = (rand() % k) + 1) != c[bestNode])
        ; // TBD: like this in original code; check logic!
      //
      {
        numConfChecks += 2;
        bestValue = totalConflicts + conflicts[bestColor][bestNode] -
                    conflicts[c[bestNode]][bestNode];
      }
    }
    // Now execute the move
    if (verbose >= 2 && totalIterations % 1000 == 0)
      cout << "          -> Iteration " << totalIterations
           << " Cost = " << totalConflicts << endl;

    int tTenure = tabuTenure;
    if (randomTenure == 1) tTenure = (rand() % tTenure) + 1;
    moveNodeToColorForTabu(bestNode, bestColor, g, c, nodesByColor, conflicts,
                           nbcPosition, neighbors, nodesInConflict,
                           confPosition, tabuStatus, totalIterations, tTenure);
    ...
    if (staticTenure == 0) {
      ...
      if (currentIterations % frequency == 0) {
        ...
        if (maxSolutionValue - minSolutionValue < 4 + totalConflicts / 80 ||
            tabuTenure <= 1) {
    ...
    if (totalConflicts < bestSolutionValue) {
      ...
      if (bestSolutionValue == 0) {
        // We have found a feasible solution with k colours, so we jump out of
        // the tabu loop
        break;
      }
    ...
    if (totalIterations == nextVerbose && incVerbose) {
      cout << totalIterations << "   obj =" << totalConflicts
           << "   best =" << bestSolutionValue << "   tenure =" << tabuTenure
           << endl;
  ...
  vstream(2) << "          -> Iteration " << totalIterations
             << " Cost = " << totalConflicts << endl;
